...
};
{
    var tests = function (it) {
        it('throws error with invalid input', function (math) {
            var x = ndarray_1.Array2D.new([1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 3;
            expect(function () { return math.localResponseNormalization3D(x, radius); })
                .toThrowError();
        });
        it('throws error with invalid radius', function (math) {
            var x = ndarray_1.Array3D.new([1, 1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 0.5;
            expect(function () { return math.localResponseNormalization3D(x, radius); })
                .toThrowError();
        });
        it('computes simple normalization across channels', function (math) {
            var x = ndarray_1.Array3D.new([1, 1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 1;
            var bias = 1;
            var alpha = 1;
            var beta = 0.5;
            var result = math.localResponseNormalization3D(x, radius, bias, alpha, beta);
            var f = function () {
                var vals = [];
        ...
        it('uses beta = 1.0 to test GPU optimization', function (math) {
            var x = ndarray_1.Array3D.new([1, 1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 1;
            var bias = 1;
            var alpha = 1;
        ...
        it('uses beta = 0.75 to test GPU optimization', function (math) {
            var x = ndarray_1.Array3D.new([1, 1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 1;
            var bias = 1;
            var alpha = 1;
        ...
        it('computes complex normalization across channels', function (math) {
            var x = ndarray_1.Array3D.new([2, 2, 4], new Float32Array([
            ...
            var alpha = 1;
            var beta = 0.5;
            var normRegion = "acrossChannels";
            var result = math.localResponseNormalization3D(x, radius, bias, alpha, beta, normRegion);
            var f = function () {
                var vals = [];
                for (var _i = 0; _i < arguments.length; _i++) {
                    vals[_i] = arguments[_i];
        ...
        it('computes simple normalization within channel', function (math) {
            var x = ndarray_1.Array3D.new([2, 2, 1], new Float32Array([1, 20, 300, 4]));
            var radius = 1;
            var bias = 1;
            var alpha = 1;
            var beta = 0.5;
            var normRegion = "withinChannel";
            var result = math.localResponseNormalization3D(x, radius, bias, alpha, beta, normRegion);
            var f = function () {
                var vals = [];
                for (var _i = 0; _i < arguments.length; _i++) {
            ...
            var multip = f(x.get(0, 0, 0), x.get(1, 0, 0), x.get(0, 1, 0), x.get(1, 1, 0));
            test_util.expectArraysClose(result, [
                x.get(0, 0, 0) * multip,
                x.get(0, 1, 0) * multip,
                x.get(1, 0, 0) * multip,
        ...
        it('computes complex normalization within channel', function (math) {
            var x = ndarray_1.Array3D.new([3, 3, 2], new Float32Array([
            ...
            var d1 = 1;
            test_util.expectArraysClose(result, [
                x.get(0, 0, d0) * f(x.get(0, 0, d0), x.get(1, 0, d0), x.get(0, 1, d0), x.get(1, 1, d0)),
                x.get(0, 0, d1) * f(x.get(0, 0, d1), x.get(1, 0, d1), x.get(0, 1, d1), x.get(1, 1, d1)),
                x.get(0, 1, d0) * f(x.get(0, 0, d0), x.get(1, 0, d0), x.get(0, 1, d0), x.get(1, 1, d0), x.get(0, 2, d0), x.get(1, 2, d0)),
                x.get(0, 1, d1) * f(x.get(0, 0, d1), x.get(1, 0, d1), x.get(0, 1, d1), x.get(1, 1, d1), x.get(0, 2, d1), x.get(1, 2, d1)),
                x.get(0, 2, d0) * f(x.get(0, 1, d0), x.get(1, 1, d0), x.get(0, 2, d0), x.get(1, 2, d0)),
                x.get(0, 2, d1) * f(x.get(0, 1, d1), x.get(1, 1, d1), x.get(0, 2, d1), x.get(1, 2, d1)),
                x.get(1, 0, d0) * f(x.get(0, 0, d0), x.get(1, 0, d0), x.get(2, 0, d0), x.get(0, 1, d0), x.get(1, 1, d0), x.get(2, 1, d0)),
                x.get(1, 0, d1) * f(x.get(0, 0, d1), x.get(1, 0, d1), x.get(2, 0, d1), x.get(0, 1, d1), x.get(1, 1, d1), x.get(2, 1, d1)),
                x.get(1, 1, d0) * f(x.get(0, 0, d0), x.get(1, 0, d0), x.get(2, 0, d0), x.get(0, 1, d0), x.get(1, 1, d0), x.get(2, 1, d0), x.get(0, 2, d0), x.get(1, 2, d0), x.get(2, 2, d0)),
                x.get(1, 1, d1) * f(x.get(0, 0, d1), x.get(1, 0, d1), x.get(2, 0, d1), x.get(0, 1, d1), x.get(1, 1, d1), x.get(2, 1, d1), x.get(0, 2, d1), x.get(1, 2, d1), x.get(2, 2, d1)),
                x.get(1, 2, d0) * f(x.get(0, 1, d0), x.get(1, 1, d0), x.get(2, 1, d0), x.get(0, 2, d0), x.get(1, 2, d0), x.get(2, 2, d0)),
                x.get(1, 2, d1) * f(x.get(0, 1, d1), x.get(1, 1, d1), x.get(2, 1, d1), x.get(0, 2, d1), x.get(1, 2, d1), x.get(2, 2, d1)),
                x.get(2, 0, d0) * f(x.get(1, 0, d0), x.get(2, 0, d0), x.get(1, 1, d0), x.get(2, 1, d0)),
                x.get(2, 0, d1) * f(x.get(1, 0, d1), x.get(2, 0, d1), x.get(1, 1, d1), x.get(2, 1, d1)),
                x.get(2, 1, d0) * f(x.get(1, 0, d0), x.get(2, 0, d0), x.get(1, 1, d0), x.get(2, 1, d0), x.get(1, 2, d0), x.get(2, 2, d0)),
                x.get(2, 1, d1) * f(x.get(1, 0, d1), x.get(2, 0, d1), x.get(1, 1, d1), x.get(2, 1, d1), x.get(1, 2, d1), x.get(2, 2, d1)),
                x.get(2, 2, d0) * f(x.get(1, 1, d0), x.get(2, 1, d0), x.get(1, 2, d0), x.get(2, 2, d0)),
                x.get(2, 2, d1) * f(x.get(1, 1, d1), x.get(2, 1, d1), x.get(1, 2, d1), x.get(2, 2, d1)),
            ]);
        });
        it('yields same result as tensorflow', function (math) {
            var input = [
            ...
            var x = ndarray_1.Array3D.new([3, 3, 8], new Float32Array(flatten(input)));
            var radius = 2;
            var bias = 1;
            var alpha = 1;
            var beta = 0.5;
...
}
{
    var tests = function (it) {
        it('throws error with invalid input', function (math) {
            var x = ndarray_1.Array3D.new([1, 1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 3;
            expect(function () { return math.localResponseNormalization4D(x, radius); })
                .toThrowError();
        });
        it('throws error with invalid radius', function (math) {
            var x = ndarray_1.Array4D.new([1, 1, 1, 4], new Float32Array([1, 20, 300, 4]));
            var radius = 0.5;
            expect(function () { return math.localResponseNormalization4D(x, radius); })
                .toThrowError();
        });
        it('computes simple normalization across channels', function (math) {
            var x = ndarray_1.Array4D.new([2, 1, 1, 4], new Float32Array([1, 20, 300, 4, 1, 20, 300, 4]));
            var radius = 1;
            var bias = 1;
            var alpha = 1;
            var beta = 0.5;
            var result = math.localResponseNormalization4D(x, radius, bias, alpha, beta);
            var f = function () {
                var vals = [];
                for (var _i = 0; _i < arguments.length; _i++) {
                    vals[_i] = arguments[_i];
        ...
        it('computes simple normalization within channel', function (math) {
            var x = ndarray_1.Array4D.new([2, 2, 2, 1], new Float32Array([1, 20, 50, 4, 1, 20, 50, 4]));
            var radius = 1;
            var bias = 1;
            var alpha = 1;
            var beta = 0.5;
            var normRegion = "withinChannel";
            var result = math.localResponseNormalization4D(x, radius, bias, alpha, beta, normRegion);
            var f = function () {
                var vals = [];
                for (var _i = 0; _i < arguments.length; _i++) {
                    vals[_i] = arguments[_i];
            ...
            var multip = function (b, d) { return f(x.get(b, 0, 0, d), x.get(b, 1, 0, d), x.get(b, 0, 1, d), x.get(b, 1, 1, d)); };
            var b0 = 0;
            var b1 = 1;
            test_util.expectArraysClose(result, [
        ...
        it('yields same result as tensorflow', function (math) {
            ...
            var x = ndarray_1.Array4D.new([2, 3, 3, 8], new Float32Array(flatten(input)));
            var radius = 2;
            var result = math.localResponseNormalization4D(x, radius);
            test_util.expectArraysClose(result, flatten(expected));
        });
    };
