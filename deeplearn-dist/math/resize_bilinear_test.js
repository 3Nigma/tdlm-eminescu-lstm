...
var ndarray_1 = require("./ndarray");
{
    var tests = function (it) {
        it('simple alignCorners=false', function (math) {
            var input = ndarray_1.Array3D.new([2, 2, 1], [2, 2, 4, 4]);
            var output = input.resizeBilinear([3, 3], false);
            test_util.expectArraysClose(output, [2, 2, 2, 10 / 3, 10 / 3, 10 / 3, 4, 4, 4]);
        });
        it('simple alignCorners=true', function (math) {
            var input = ndarray_1.Array3D.new([2, 2, 1], [2, 2, 4, 4]);
            var output = input.resizeBilinear([3, 3], true);
            test_util.expectArraysClose(output, [2, 2, 2, 3, 3, 3, 4, 4, 4]);
        });
        it('matches tensorflow w/ random numbers alignCorners=false', function (math) {
            var input = ndarray_1.Array3D.new([2, 3, 2], [
        ...
        it('matches tensorflow w/ random numbers alignCorners=true', function (math) {
            var input = ndarray_1.Array3D.new([2, 3, 2], [
        ...
        it('batch of 2, simple, alignCorners=true', function (math) {
            var input = ndarray_1.Array4D.new([2, 2, 2, 1], [2, 2, 4, 4, 3, 3, 5, 5]);
            var output = input.resizeBilinear([3, 3], true);
            test_util.expectArraysClose(output, [2, 2, 2, 3, 3, 3, 4, 4, 4, 3, 3, 3, 4, 4, 4, 5, 5, 5]);
        });
    };
