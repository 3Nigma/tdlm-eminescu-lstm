...
describe('conv_util computeConvInfo', function () {
    it('1x1 conv over 1x1 array with same pad', function () {
        var inShape = [1, 1, 1, 1];
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [1, 1, 1, 1], stride, 'same');
    ...
    it('2x2 conv over 3x3 array with same pad', function () {
        var inShape = [1, 3, 3, 1];
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [2, 2, 1, 1], stride, 'same');
        expect(convInfo.batchSize).toEqual(1);
    ...
    it('2x2 conv over 3x3 array with valid pad', function () {
        var inShape = [1, 3, 3, 1];
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [2, 2, 1, 1], stride, 'valid');
        expect(convInfo.batchSize).toEqual(1);
    ...
    it('2x1 conv over 3x3 array with valid pad with stride 1', function () {
        var inShape = [1, 3, 3, 1];
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [2, 1, 1, 1], stride, 'valid');
        expect(convInfo.batchSize).toEqual(1);
        expect(convInfo.outHeight).toEqual(2);
        expect(convInfo.outWidth).toEqual(3);
        expect(convInfo.outChannels).toEqual(1);
    ...
    it('2x1 conv over 3x3 array with valid pad with strides h=2, w=1', function () {
        var inShape = [1, 3, 3, 1];
        var strides = [2, 1];
        var convInfo = conv_util.computeConv2DInfo(inShape, [2, 1, 1, 1], strides, 'valid');
        expect(convInfo.batchSize).toEqual(1);
        expect(convInfo.outHeight).toEqual(1);
        expect(convInfo.outWidth).toEqual(3);
        expect(convInfo.outChannels).toEqual(1);
    ...
    it('1x2 conv over 3x3 array with valid pad with stride 1', function () {
        var inShape = [1, 3, 3, 1];
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [1, 2, 1, 1], stride, 'valid');
        expect(convInfo.batchSize).toEqual(1);
        expect(convInfo.outHeight).toEqual(3);
        expect(convInfo.outWidth).toEqual(2);
        expect(convInfo.outChannels).toEqual(1);
...
describe('conv_util computeConv2DInfo with depthwise=true', function () {
    it('1x1 filter over 1x1 array with same pad', function () {
        ...
        var stride = 1;
        var pad = 'same';
        var convInfo = conv_util.computeConv2DInfo(inShape, [fSize, fSize, inChannels, chMul], stride, pad, null, true);
        expect(convInfo.batchSize).toEqual(1);
        expect(convInfo.outHeight).toEqual(1);
        expect(convInfo.outWidth).toEqual(1);
        expect(convInfo.outChannels).toEqual(1);
...
describe('conv_util computeConvInfo channelsFirst', function () {
    it('2x2 conv over 3x3 array with same pad', function () {
        ...
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [2, 2, inDepth, outDepth], stride, 'same', null, false, 'channelsFirst');
        expect(convInfo.batchSize).toEqual(1);
        expect(convInfo.outHeight).toEqual(3);
        expect(convInfo.outWidth).toEqual(3);
    ...
    it('2x2 conv over 3x3 array with valid pad', function () {
        ...
        var inShape = [1, inDepth, 3, 3];
        var stride = 1;
        var convInfo = conv_util.computeConv2DInfo(inShape, [2, 2, inDepth, outDepth], stride, 'valid', null, false, 'channelsFirst');
        expect(convInfo.batchSize).toEqual(1);
        expect(convInfo.outHeight).toEqual(2);
        expect(convInfo.outWidth).toEqual(2);
...
describe('conv_util computeConvInfo roundingMode', function () {
    ...
    it('should fail computing the output dimension of Conv Layer', function () {
        expect(function () { return conv_util.computeConv2DInfo(inShape, [fSize, fSize, inChannels, chMul], stride, pad); })
            .toThrowError();
    });
    it('Floor the output dimension of Conv Layer', function () {
        var convInfo = conv_util.computeConv2DInfo(inShape, [fSize, fSize, inChannels, chMul], stride, pad, 'floor');
        expect(convInfo.outShape).toEqual([batchSize, 3, 3, chMul]);
    });
    it('Round the output dimension of Conv Layer', function () {
        var convInfo = conv_util.computeConv2DInfo(inShape, [fSize, fSize, inChannels, chMul], stride, pad, 'round');
        expect(convInfo.outShape).toEqual([batchSize, 4, 4, chMul]);
    });
    it('Ceil the output dimension of Conv Layer', function () {
        var convInfo = conv_util.computeConv2DInfo(inShape, [fSize, fSize, inChannels, chMul], stride, pad, 'ceil');
        expect(convInfo.outShape).toEqual([batchSize, 4, 4, chMul]);
    });
});
