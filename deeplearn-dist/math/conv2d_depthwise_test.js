...
var ndarray_1 = require("./ndarray");
{
    var tests = function (it) {
        it('input=1x3x3x1,f=2,s=1,p=valid,chMul=1', function (math) {
            var fSize = 2;
            var pad = 'valid';
            var stride = 1;
            var chMul = 1;
            var inDepth = 1;
            var x = ndarray_1.Array4D.new([1, 3, 3, inDepth], [
                0.230664, 0.987388, 0.0685208, 0.419224, 0.887861, 0.731641, 0.0741907,
                0.409265, 0.351377
            ]);
            var w = ndarray_1.Array4D.new([fSize, fSize, inDepth, chMul], [0.303873, 0.229223, 0.144333, 0.803373]);
            var result = math.depthwiseConv2D(x, w, stride, pad);
            expect(result.shape).toEqual([1, 2, 2, 1]);
            var expected = [1.07022, 1.03167, 0.67041, 0.778863];
            test_util.expectArraysClose(result, expected);
        });
        it('input=1x3x3x2,f=2,s=1,p=same,chMul=1', function (math) {
            var fSize = 2;
            var pad = 'same';
            var stride = 1;
            var chMul = 1;
            ...
            var w = ndarray_1.Array4D.new([fSize, fSize, inDepth, chMul], [
        ...
        it('input=1x3x3x2,f=2,s=1,p=same,chMul=2', function (math) {
            var fSize = 2;
            var pad = 'same';
            var stride = 1;
            var chMul = 2;
        ...
        it('input=2x3x3x2,f=2,s=1,p=same,chMul=2', function (math) {
            var fSize = 2;
            var pad = 'same';
            var stride = 1;
            var chMul = 2;
            var inDepth = 2;
            var x = ndarray_1.Array4D.new([2, 3, 3, inDepth], [
        ...
        it('Array3D is allowed', function (math) {
            var fSize = 2;
            var pad = 'same';
            var stride = 1;
            var chMul = 3;
            var inDepth = 2;
            var x = dl.zeros([3, 3, inDepth]);
            var w = dl.zeros([fSize, fSize, inDepth, chMul]);
            var result = math.depthwiseConv2D(x, w, stride, pad);
